function init_include()
	...
	state.ElementalWheel 	  = M(false, 'Elemental Wheel') ---turning this on rotates the spells
...
function get_idle_set(petStatus)
    ...
    -- [NOTE] This user_customize/job_customize/user_job_customize 3-hook cascade repeats
    -- verbatim (just the hook name changes) in get_melee_set, apply_defense, apply_passive,
    -- and apply_kiting below -- 5 copies total. Could collapse into one shared helper
    -- (e.g. apply_customize_hooks(baseSet, 'idle_set')) if this file ever gets refactored.
    -- Purely stylistic -- all 5 copies are correct and consistent as-is.
...
function get_melee_set()
    ...
    -- [NOTE] Same 3-hook cascade as get_idle_set above (2 of 5 copies) -- see note there.
...
function apply_defense(baseSet)
    if state.DefenseMode.current ~= 'None' then
        ...
        -- [NOTE] Same 3-hook cascade as get_idle_set above (3 of 5 copies) -- see note there.
...
function apply_passive(baseSet)
	...
	-- [NOTE] Same 3-hook cascade as get_idle_set above (4 of 5 copies) -- see note there.
...
function apply_kiting(baseSet)
	...
	-- [NOTE] Same 3-hook cascade as get_idle_set above (5 of 5 copies, last one) -- see note there.
