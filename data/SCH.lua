...
--             Sublimation auto-reactivation) and added handle_smartcure(). Note: SCH already
    ...
    elseif spell.skill == 'Elemental Magic' and spell.english ~= 'Impact' then
		...
		if not state.CastingMode.value:contains('Resistant') then
			if spell.element == world.weather_element or spell.element == world.day_element then
				...
				-- end
				if spell.element == world.day_element and (state.CastingMode.value == 'Fodder' or state.CastingMode.value == 'Normal') then
...
function job_aftercast(spell, spellMap, eventArgs)
    ...
    if not spell.interrupted then
		...
		elseif spell.action_type == 'Magic' then
			...
			-- [FIX 2026-08-30] Removed the "reset MagicBurstMode after one Elemental Magic
			-- cast in Single mode" branch here -- MagicBurstMode is retired, folded into
			-- CastingMode. MB is now a PERSISTENT toggle per direct instruction; nothing
			-- auto-reverts it anymore.
			end
		end
    end
end
...
function handle_elemental(cmdParams)
	...
	elseif command:contains('tier') then
		...
		elseif requested > MaxNukeTier then
			add_to_chat(123,'Abort: SCH is capped at Tier '..MaxNukeTier..' nukes -- Tier '..requested..' is not available.')
		else
